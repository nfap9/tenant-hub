...
- **API Dockerfile**（`apps/api/Dockerfile`）：多阶段构建（base → deps → development → build → production）
  ...
  - 暴露端口 `4000`
- **Tenant Web Dockerfile**（`apps/tenant-web/Dockerfile`）：多阶段构建，最终用 nginx:1.27-alpine 托管静态资源
  - 构建参数 `VITE_API_BASE_URL`
  - 暴露端口 `80`
...
- **`docker-compose.prod.yml`**：生产编排
  ...
  - `tenant-web` 服务：端口 `127.0.0.1:3000:80`，healthcheck `/`
