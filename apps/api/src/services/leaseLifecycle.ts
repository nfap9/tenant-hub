...
export const isAutoRenewalPeriod = (lease: RenewalStateLease, today = new Date()) =>
  lease.autoRenew && lease.status === "ACTIVE" && startOfLeaseDay(today).isAfter(startOfLeaseDay(lease.endDate), "day");

export const getLeaseBillGenerationEnd = (lease: BillGenerationLease, today = new Date()) => {
...
export const assertExpiredTerminationAllowed = (endDate: Date, terminatedAt: Date) => {
  if (startOfLeaseDay(terminatedAt).isBefore(startOfLeaseDay(endDate), "day")) {
    throw new Error("到期解约的退租日期不能早于原租约结束日期");
  }
};

export const withLeaseLifecycle = <T extends RenewalStateLease>(lease: T, today = new Date()) => ({
  ...lease,
