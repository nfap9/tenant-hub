...
describe("prisma config", () => {
  ...
  it("should soft-delete on delete action", async () => {
    ...
    const params = { action: "delete", model: "Lease", args: { where: { id: "1" } } };
    ...
    expect(next).toHaveBeenCalledWith(
      expect.objectContaining({
        action: "update",
        args: expect.objectContaining({ data: expect.objectContaining({ deletedAt: expect.any(Date) }) })
      })
    );
  });
  ...
  it("should soft-delete on deleteMany action", async () => {
    const next = vi.fn(async (params: any) => params);
    const params = { action: "deleteMany", model: "Bill", args: { where: { status: "UNPAID" } } };
  ...
  it("should auto-filter deletedAt for findMany", async () => {
    const next = vi.fn(async (params: any) => params);
    const params = { action: "findMany", model: "Apartment", args: { where: { name: "A" } } };
  ...
  it("should not override explicit deletedAt filter", async () => {
    const next = vi.fn(async (params: any) => params);
    const params = { action: "findMany", model: "Room", args: { where: { deletedAt: new Date() } } };

    await middleware(params, next);

    expect(next).toHaveBeenCalledWith(
      expect.objectContaining({
        args: expect.objectContaining({ where: expect.objectContaining({ deletedAt: expect.any(Date) }) })
      })
    );
  });
  ...
  it("should not touch non-soft-delete models", async () => {
    const next = vi.fn(async (params: any) => params);
    const params = { action: "delete", model: "User", args: { where: { id: "1" } } };
