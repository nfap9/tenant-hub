...
vi.mock("../../src/services/billing.js", () => ({
  ...
  getCurrentMonthBillWindow: vi.fn(() => ({ start: new Date(), end: new Date() })),
...
describe("bills routes", () => {
  ...
  describe("GET /api/bills/meter-readings", () => {
    it("should return meter readings", async () => {
      (prisma.meterReading.findMany as ReturnType<typeof vi.fn>).mockResolvedValue([{ id: "mr-1" }]);
  ...
  describe("POST /api/bills/meter-readings", () => {
    it("should create a meter reading", async () => {
      (prisma.room.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "room-1", apartmentId: "apt-1" });
      (prisma.lease.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "lease-1" });
      (prisma.meterReading.create as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "mr-1" });
      (prisma.bill.findMany as ReturnType<typeof vi.fn>).mockResolvedValue([]);
      ...
      const res = await request(app)
        ...
        .set("Authorization", `Bearer ${authToken}`)
        .set("x-organization-id", "org-1")
        .send({ roomId: "room-1", meterType: "WATER", readingDate: "2026-05-01", value: 100 });

      expect(res.status).toBe(200);
    });

    it("should return 404 for non-existent room", async () => {
      (prisma.room.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue(null);
  ...
  describe("POST /api/bills/monthly/:id/payments", () => {
    it("should record a monthly bill payment", async () => {
      (prisma.monthlyBill.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "mb-1" });
    ...
    it("should return 404 for missing monthly bill", async () => {
      (prisma.monthlyBill.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue(null);
  ...
  describe("POST /api/bills/:id/retry-billing", () => {
    it("should retry billing for postpaid bill", async () => {
      (prisma.bill.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "bill-1", mode: "POSTPAID" });
    ...
    it("should reject retry for non-postpaid bill", async () => {
      (prisma.bill.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "bill-1", mode: "PREPAID" });
  ...
  describe("DELETE /api/bills/monthly/:id", () => {
    it("should delete a monthly bill", async () => {
      (prisma.monthlyBill.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue({
        id: "mb-1",
        status: "UNPAID",
        bills: [{ id: "bill-1" }],
        payments: []
