  ...
  prisma: {
    $transaction: vi.fn(async (callback: any) => {
      if (typeof callback === "function") {
        return callback({
          orgMember: { findUnique: vi.fn(), count: vi.fn(), upsert: vi.fn(async () => ({ id: "member-1" })) },
...
describe("organizations routes", () => {
  beforeEach(() => {
    ...
    (prisma.orgMember.findMany as ReturnType<typeof vi.fn>).mockResolvedValue([
      { organization: { id: "org-1", name: "测试组织" }, role: { code: "owner" } }
    ]);
  });
  ...
  describe("POST /api/organizations", () => {
    it("should create an organization", async () => {
      (prisma.role.findUniqueOrThrow as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "role-owner", code: "owner" });
      (prisma.organization.create as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "org-1", name: "新组织" });
  ...
  describe("POST /api/organizations/join", () => {
    it("should join by invite code", async () => {
      (prisma.orgInvite.findUnique as ReturnType<typeof vi.fn>).mockResolvedValue({
        id: "invite-1",
        code: "ABC123",
      ...
      (prisma.role.findUniqueOrThrow as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "role-readonly", code: "readonly" });
    ...
    it("should return 404 for invalid invite code", async () => {
      (prisma.orgInvite.findUnique as ReturnType<typeof vi.fn>).mockResolvedValue(null);
  ...
  describe("GET /api/organizations/:organizationId/invites", () => {
    it("should return invites", async () => {
      ...
      (prisma.orgInvite.findMany as ReturnType<typeof vi.fn>).mockResolvedValue([{ id: "i-1" }]);
  ...
  describe("POST /api/organizations/:organizationId/invites", () => {
    it("should create an invite", async () => {
      ...
      (prisma.orgInvite.create as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "i-1", code: "ABC123" });
  ...
  describe("GET /api/organizations/plans", () => {
    it("should return enabled plans", async () => {
      (prisma.plan.findMany as ReturnType<typeof vi.fn>).mockResolvedValue([{ id: "plan-1", enabled: true }]);
  ...
  describe("GET /api/organizations/:organizationId/subscription", () => {
    it("should return subscription info", async () => {
      ...
      (prisma.organization.findUniqueOrThrow as ReturnType<typeof vi.fn>).mockResolvedValue({
        _count: { apartments: 0, members: 0 }
      });
      (prisma.orgQuotaPackage.findMany as ReturnType<typeof vi.fn>).mockResolvedValue([]);
  ...
  describe("POST /api/organizations/:organizationId/subscriptions", () => {
    it("should subscribe to a plan", async () => {
      ...
      (prisma.plan.findUnique as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "plan-1", enabled: true });
      (prisma.subscription.create as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "sub-1", plan: { id: "plan-1" } });
    ...
    it("should reject disabled plan", async () => {
      ...
      (prisma.plan.findUnique as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "plan-1", enabled: false });
  ...
  describe("PUT /api/organizations/:organizationId", () => {
    it("should update organization", async () => {
      ...
      (prisma.organization.update as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "org-1", name: "新名字" });
  ...
  describe("DELETE /api/organizations/:organizationId", () => {
    it("should delete organization with confirmation", async () => {
      ...
      (prisma.organization.update as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "org-1", status: "DELETED" });
  ...
  describe("GET /api/organizations/:organizationId/roles", () => {
    it("should return roles", async () => {
      ...
      (prisma.role.findMany as ReturnType<typeof vi.fn>).mockResolvedValue([{ id: "r-1" }]);
  ...
  describe("GET /api/organizations/:organizationId/members", () => {
    it("should return members", async () => {
      ...
      (prisma.orgMember.findMany as ReturnType<typeof vi.fn>).mockResolvedValue([{ id: "m-1" }]);
  ...
  describe("DELETE /api/organizations/:organizationId/members/:memberId", () => {
    it("should remove a member", async () => {
      ...
      (prisma.orgMember.findUniqueOrThrow as ReturnType<typeof vi.fn>).mockResolvedValue({
        id: "m-2",
        organizationId: "org-1",
        role: { code: "manager" }
      });
      (prisma.orgMember.update as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "m-2", status: "DISABLED" });
  ...
  describe("PUT /api/organizations/:organizationId/members/:memberId/role", () => {
    it("should change member role", async () => {
      ...
      (prisma.role.findUniqueOrThrow as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "role-owner", code: "owner" });
      (prisma.orgMember.update as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "m-2", roleId: "new-role" });
