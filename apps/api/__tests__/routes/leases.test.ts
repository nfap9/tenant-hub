...
describe("leases routes", () => {
  ...
  describe("GET /api/leases", () => {
    it("should return leases for organization", async () => {
      (prisma.lease.findMany as ReturnType<typeof vi.fn>).mockResolvedValue([{ id: "lease-1" }]);
  ...
  describe("POST /api/leases", () => {
    it("should create a lease", async () => {
      (prisma.room.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "room-1", status: "VACANT" });
      (prisma.lease.create as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "lease-1", roomId: "room-1" });
    ...
    it("should reject non-vacant room", async () => {
      (prisma.room.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "room-1", status: "OCCUPIED" });
  ...
  describe("PUT /api/leases/:id", () => {
    it("should update a lease", async () => {
      ...
      (prisma.lease.update as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "lease-1", rentAmount: 1200 });
  ...
  describe("POST /api/leases/:id/terminate", () => {
    it("should terminate a lease", async () => {
      (prisma.lease.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "lease-1", endDate: new Date() });
    ...
    it("should return 404 for missing lease", async () => {
      (prisma.lease.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue(null);
  ...
  describe("GET /api/leases/settlements", () => {
    it("should return settlements", async () => {
      (prisma.leaseSettlement.findMany as ReturnType<typeof vi.fn>).mockResolvedValue([{ id: "s-1" }]);
