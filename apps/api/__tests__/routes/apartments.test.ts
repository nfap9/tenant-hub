...
vi.mock("../../src/services/billing.js", () => ({
  getCurrentMonthBillWindow: vi.fn(() => ({ start: new Date("2026-07-01"), end: new Date("2026-08-01") })),
...
describe("apartments routes", () => {
  ...
  describe("PUT /api/apartments/:id", () => {
    it("should update an apartment", async () => {
      (prisma.apartment.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "apt-1" });
      (prisma.apartment.update as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "apt-1", name: "新名字" });
    ...
    it("should return 404 for non-existent apartment", async () => {
      (prisma.apartment.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue(null);
  ...
  describe("DELETE /api/apartments/:id", () => {
    it("should delete an apartment without active leases", async () => {
      ...
      (prisma.apartment.delete as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "apt-1" });
  ...
  describe("POST /api/apartments/:id/expenses", () => {
    it("should create an expense", async () => {
      (prisma.apartment.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "apt-1" });
      (prisma.apartmentExpense.create as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "exp-1", name: "维修费", amount: 500 });
  ...
  describe("PUT /api/apartments/rooms/:roomId", () => {
    it("should update a room", async () => {
      (prisma.room.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "room-1" });
      (prisma.room.update as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "room-1", roomNo: "101A" });
  ...
  describe("DELETE /api/apartments/rooms/:roomId", () => {
    it("should delete a room without active leases", async () => {
      (prisma.room.findFirst as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "room-1" });
      (prisma.lease.count as ReturnType<typeof vi.fn>).mockResolvedValue(0);
      (prisma.room.delete as ReturnType<typeof vi.fn>).mockResolvedValue({ id: "room-1" });
