...
describe("requireAuth", () => {
  ...
  it("should throw 401 when no token", () => {
    ...
    expect(() => requireAuth(req, res, next)).toThrow(expect.objectContaining({ status: 401 }));
  });

  it("should set req.user on valid token", async () => {
    const token = jwt.sign({ id: "user-1", phone: "13800138000", username: "测试用户" }, secret);
    ...
    expect(req.user).toEqual({ id: "user-1", phone: "13800138000", username: "测试用户" });
...
describe("requireOrg", () => {
  ...
  it("should set req.organizationId and req.permissions on valid member", async () => {
    const req = mockReq({ user: { id: "user-1" }, headers: { "x-organization-id": "org-1" } });
    const res: any = {};
    const next = vi.fn();
...
describe("requirePermission", () => {
  ...
  it("should throw 403 when permission missing", () => {
    ...
    expect(() => requirePermission("LEASE_MANAGE")(req, res, next)).toThrow(expect.objectContaining({ status: 403 }));
  });
