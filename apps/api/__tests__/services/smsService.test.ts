...
describe('sms service', () => {
  ...
  it('should replace template variables in URL', async () => {
    ...
    await sendSms({
      ...
      number: 5,
      config: {
        url: 'https://sms.example.com/send?code={{code}}&phone={{targets}}',
        method: 'GET',
      },
    });
    ...
    expect(url).toBe(
      'https://sms.example.com/send?code=123456&phone=13800000000'
    );
  });
  ...
  it('should send POST request with JSON body by default', async () => {
    ...
    expect(options.body).toBe(
      JSON.stringify({ code: '123456', phone: '13800000000' })
    );
  });
  ...
  it('should join multiple targets with comma', async () => {
    ...
    await sendSms({
      ...
      code: '123456',
      config: {
        url: 'https://sms.example.com/send?phone={{targets}}',
        method: 'GET',
      },
    });
  ...
  it('should throw on non-ok response', async () => {
    ...
    await expect(
      sendSms({
        targets: '13800000000',
        code: '123456',
        config: { url: 'https://sms.example.com/send', method: 'POST' },
      })
