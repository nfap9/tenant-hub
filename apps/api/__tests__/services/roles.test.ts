...
describe("roles", () => {
  ...
  it("should upsert owner role with all permissions", async () => {
    ...
    const ownerCall = mockUpsert.mock.calls.find((call) => call[0].where.code === "owner");
  ...
  it("should upsert manager role with limited permissions", async () => {
    ...
    const managerCall = mockUpsert.mock.calls.find((call) => call[0].where.code === "manager");
  ...
  it("should upsert readonly role with view-only permissions", async () => {
    ...
    const readonlyCall = mockUpsert.mock.calls.find((call) => call[0].where.code === "readonly");
