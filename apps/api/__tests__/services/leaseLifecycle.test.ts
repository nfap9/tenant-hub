...
import {
  ...
  isAutoRenewalPeriod,
} from '../../src/services/leaseLifecycle.js';
...
describe('lease lifecycle', () => {
  it('should detect renewal period for active auto-renew lease after end date', () => {
    expect(
      isAutoRenewalPeriod(
        { autoRenew: true, status: 'ACTIVE', endDate: date('2026-05-01') },
        date('2026-05-02')
      )
    ).toBe(true);
  });
  ...
  it('should not detect renewal period for non-auto-renew lease', () => {
    expect(
      isAutoRenewalPeriod(
        { autoRenew: false, status: 'ACTIVE', endDate: date('2026-05-01') },
        date('2026-05-02')
      )
    ).toBe(false);
  });
  ...
  it('should allow expired termination on or after original end date', () => {
    expect(() =>
      assertExpiredTerminationAllowed(date('2026-05-10'), date('2026-05-10'))
    ).not.toThrow();
    expect(() =>
      assertExpiredTerminationAllowed(date('2026-05-10'), date('2026-06-01'))
    ).not.toThrow();
  });
