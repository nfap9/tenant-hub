...
import { enforceOrganizationQuota, assertOrganizationQuota, getOrganizationQuota, isQuotaLimitEnabled } from "../../src/services/quotas.js";
...
describe("quotas", () => {
  ...
  it("should throw when quota exceeds limit", async () => {
    ...
    await expect(assertOrganizationQuota("org-1", "apartment", 3, db as never)).rejects.toThrow(/公寓数量已达到套餐额度上限/);
  });
  ...
  it("should throw when no active subscription and quota enabled", async () => {
    ...
    await expect(assertOrganizationQuota("org-1", "room", 1, db as never)).rejects.toThrow(/请先购买套餐/);
  });
  ...
  it("should enforce quota with lock when enabled", async () => {
    ...
    await expect(enforceOrganizationQuota(db as never, "org-1", "room", getNextCount)).resolves.toBeUndefined();
