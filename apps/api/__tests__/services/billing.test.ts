...
describe('billing', () => {
  it('should calculate billing periods correctly', () => {
    ...
    expect(periods.prepaid.start.toISOString()).toBe(
      date('2026-06-05').toISOString()
    );
    expect(periods.prepaid.end.toISOString()).toBe(
      date('2026-07-04').toISOString()
    );
    expect(periods.postpaid.start.toISOString()).toBe(
      date('2026-05-05').toISOString()
    );
    expect(periods.postpaid.end.toISOString()).toBe(
      date('2026-06-04').toISOString()
    );
  });
  ...
  it('should generate bills for every current lease', async () => {
    const result = await generateCurrentLeaseBills(
      'org-a',
      date('2026-07-10'),
      {
        findCurrentLeases: async (organizationId) => {
          expect(organizationId).toBe('org-a');
          return [{ id: 'lease-a' }, { id: 'lease-b' }] as unknown as Awaited<
            ReturnType<
              Parameters<
                typeof generateCurrentLeaseBills
              >[2]['findCurrentLeases']
            >
          >;
        },
    ...
    expect(result).toEqual({
      leaseCount: 2,
      billIds: ['bill-a-1', 'bill-a-2', 'bill-b-1'],
    });
  });
    ...
    const result = await generateCurrentLeaseBills(
      ...
      date('2026-07-10'),
      {
        ...
        generateLeaseBillsForLease: async () => {
          throw new Error(
            'should not generate bills when there are no current leases'
          );
        },
      }
    );
  ...
  it('should define current month bill window correctly', () => {
    ...
    expect(currentMonthWindow.start.toISOString()).toBe(
      date('2026-07-01').toISOString()
    );
    expect(currentMonthWindow.end.toISOString()).toBe(
      date('2026-08-01').toISOString()
    );
  });
