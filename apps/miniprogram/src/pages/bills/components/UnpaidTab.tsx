...
export function UnpaidTab({ bills, onDetail }: UnpaidTabProps) {
  const sorted = sortMonthlyBillsForList(bills);
  return (
    <>
      {bills.length === 0 ? (
        <EmptyState icon="bill" title="暂无待支付账单" subtitle="所有账单均已结清或暂无账单" />
      ) : null}
      {sorted.map((bill) => (
        <BillCard key={bill.id} bill={bill} onClick={() => onDetail(bill)} />
      ))}
    </>
  );
}
