...
export default function PaymentPage() {
  ...
  const paymentBills = useMemo(() => sortMonthlyBillsForList(bills.filter((b) => b.status === "UNPAID" || b.status === "PARTIAL_PAID")), [bills]);
  ...
  useEffect(() => {
    if (paymentBills.length > 0 && !paymentRoomId) {
      ...
      if (first) {
        ...
        setForm({ monthlyBillId: isPayable ? first.id : "", amount: isPayable ? String(remainingAmount(first)) : "", method: "线下收款", note: "" });
      }
    }
  ...
  const handleRoomClick = (roomId: string) => {
    ...
    if (firstBill) {
      ...
      setForm({ monthlyBillId: isPayable ? firstBill.id : "", amount: isPayable ? String(remainingAmount(firstBill)) : "", method: "线下收款", note: "" });
    } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await apiClient(`/bills/monthly/${form.monthlyBillId}/payments`, {
        method: "POST",
        body: { amount: Number(form.amount), method: form.method.trim() || "线下收款", note: form.note.trim() || undefined },
  ...
  return (
    <View className="page-container">
      ...
      <Card title="登记收款">
        ...
        {roomBills.map((bill) => (
          <View key={bill.id} className={`bill-select-card ${form.monthlyBillId === bill.id ? 'bill-select-card--active' : ''}`} onClick={() => handleBillClick(bill)}>
