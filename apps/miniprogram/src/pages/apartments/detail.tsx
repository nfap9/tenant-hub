...
export default function ApartmentDetailPage() {
  ...
  const apartment = useMemo(() => apartments.find((a) => a.id === apartmentId), [apartments, apartmentId]);
  ...
  const handleDeleteApartment = async () => {
    ...
    const res = await Taro.showModal({
      title: "删除公寓",
      content: "删除后公寓及下属所有房间资料不可恢复，请确认当前公寓没有有效租约。",
      confirmText: "确认删除",
      confirmColor: "#ff4d4f"
    });
    ...
    try {
      await apiClient(`/apartments/${apartment.id}`, { method: "DELETE", organizationId: currentOrgId });
  ...
  const handleRecordExpense = () => {
    if (apartment) {
      Taro.navigateTo({ url: `/pages/apartments/expense?apartmentId=${apartment.id}` });
    }
  };

  const handleOpenRoomSingle = () => {
    if (apartment) {
      Taro.navigateTo({ url: `/pages/apartments/room-form?apartmentId=${apartment.id}` });
    }
  };

  const handleOpenRoomBatch = () => {
    if (apartment) {
      Taro.navigateTo({ url: `/pages/apartments/room-batch?apartmentId=${apartment.id}` });
    }
  };

  const handleOpenRoomEdit = (room: Room) => {
    if (apartment) {
      Taro.navigateTo({ url: `/pages/apartments/room-form?apartmentId=${apartment.id}&roomId=${room.id}` });
    }
  };
    ...
    <View className="page-container page-container--apartment-detail">
      <View className="apartment-detail-fixed">
        <View className="sub-page-header">
          <Button className="page-back-button" variant="ghost" size="small" onClick={handleBack}>‹ 返回公寓列表</Button>
        </View>
        ...
        <Card
          ...
          headerAction={canManageApartment ? (
            <View className="action-row-inline">
              <Button variant="secondary" size="small" onClick={handleEdit}>编辑</Button>
              <Button variant="danger" size="small" onClick={handleDeleteApartment}>删除</Button>
            </View>
          ) : undefined}
        >
          <View className="segment">
            <View className={`segment-item ${detailTab === "expenses" ? "segment-item--active" : ""}`} onClick={() => setDetailTab("expenses")}>
              <Text className={`segment-text ${detailTab === "expenses" ? "segment-text--active" : ""}`}>公寓详情</Text>
            </View>
            <View className={`segment-item ${detailTab === "rooms" ? "segment-item--active" : ""}`} onClick={() => setDetailTab("rooms")}>
      ...
      {detailTab === "expenses" ? (
        <View className="apartment-detail-content">
          <Card className="apartment-info-card">
            <View className="detail-panel">
              <View className="detail-row">
                ...
                <Text className="card-title">{apartment.floors} 层 · 占地 {apartment.landArea ?? "未填"}㎡ · 总 {apartment.totalArea ?? "未填"}㎡</Text>
              </View>
              <View className="detail-row">
                <Text className="text-muted">上游房东</Text>
                <Text className="card-title">{apartment.landlordName || "未维护"} · {apartment.landlordPhone || "未维护"}</Text>
              </View>
              <View className="detail-row">
              ...
              <View className="detail-row">
                ...
                <Text className="card-stat">¥{money(apartment.rentAmount)}</Text>
              </View>
              <View className="detail-row">
                <Text className="text-muted">房间数量</Text>
                <Text className="card-stat">{apartment.rooms?.length ?? 0} 间</Text>
              </View>
            </View>
          </Card>
          ...
          <Card
            ...
            headerAction={canManageApartment ? <Button variant="secondary" size="small" onClick={handleRecordExpense}>记录花费</Button> : undefined}
          >
            <ScrollView className="apartment-inner-list" scrollY>
              ...
              {(apartment.expenses ?? []).length === 0 ? <Text className="text-muted">暂无经营花费记录</Text> : null}
            </ScrollView>
          </Card>
        </View>
      ) : null}
      ...
      {detailTab === "rooms" ? (
        <Card
          ...
          headerAction={canManageRoom ? (
            <View className="action-row-inline">
              <Button variant="secondary" size="small" onClick={handleOpenRoomSingle}>新增房间</Button>
              <Button variant="secondary" size="small" onClick={handleOpenRoomBatch}>批量添加</Button>
            </View>
          ) : undefined}
        >
          <ScrollView className="apartment-inner-list" scrollY>
            {apartmentRooms.map((room) => (
              <View key={room.id} className="room-card">
                ...
                {canManageRoom ? (
                  <View className="action-row-inline">
                    <Button variant="secondary" size="small" onClick={() => handleOpenRoomEdit(room)}>编辑</Button>
                    <Button variant="danger" size="small" disabled={room.status === "OCCUPIED"} onClick={() => handleDeleteRoom(room)}>删除</Button>
                  </View>
                ) : null}
              </View>
            ))}
