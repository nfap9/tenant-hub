...
export default function PlanPage() {
  ...
  const subscribePlan = async (planId: string) => {
    ...
    try {
      ...
      Taro.showToast({
        title: e instanceof Error ? e.message : '订阅失败',
        icon: 'none',
      });
    }
  };
      ...
      <Card title="当前套餐">
        {subscription ? (
            ...
            {quotaLimitEnabled ? (
              <View className="detail-row">
                ...
                <Text className="text-muted">
                  {day(subscription.startsAt)} 至{' '}
                  {subscription.endsAt ? day(subscription.endsAt) : '永久'}
                </Text>
              </View>
            ) : null}
            <View className="detail-row">
              <Text className="text-muted">状态</Text>
              <Badge tone={subscription.active ? 'success' : 'danger'}>
                {subscription.active ? '生效中' : '已过期'}
              </Badge>
            </View>
          </>
        ) : (
          <EmptyState
            ...
            title="暂无订阅"
            subtitle={
              quotaLimitEnabled ? '请选择一个套餐订阅' : '当前系统不限量使用'
            }
          />
        )}
      </Card>
      ...
      <Card title="用量统计">
        <View className="detail-row">
          ...
          <Text className="card-title">
            {usage?.apartments ?? 0} /{' '}
            {quotaLimitEnabled
              ? (subscription?.plan.apartmentLimit ?? 0) +
                (extra?.apartmentQuota ?? 0)
              : '不限量'}
          </Text>
        </View>
        <View className="detail-row">
          ...
          <Text className="card-title">
            {usage?.members ?? 0} /{' '}
            {quotaLimitEnabled
              ? (subscription?.plan.memberLimit ?? 0) +
                (extra?.memberQuota ?? 0)
              : '不限量'}
          </Text>
        </View>
          ...
          {plans
            ...
            .map((plan) => (
              <View key={plan.id} className="plan-card">
                ...
                <Text className="text-muted">
                  公寓 {plan.apartmentLimit} · 房间 {plan.roomLimit} · 成员{' '}
                  {plan.memberLimit}
                </Text>
                <Button
                  variant="secondary"
