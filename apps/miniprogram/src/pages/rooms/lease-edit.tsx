...
import { selectableFeeTypes, feeTypeLabels, type LeaseFeeFormItem } from './constants';
...
export default function LeaseEditPage() {
  ...
  const [form, setForm] = useState({ rentAmount: "", depositAmount: "", waterUnitPrice: "0", powerUnitPrice: "0" });
  ...
  const handleSave = async () => {
    ...
    try {
      await apiClient(`/leases/${lease.id}`, {
        method: "PUT",
        body: {
          rentAmount: form.rentAmount.trim() ? Number(form.rentAmount) : undefined,
