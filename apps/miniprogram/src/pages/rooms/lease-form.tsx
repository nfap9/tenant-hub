...
export default function LeaseFormPage() {
  ...
  const updateForm = (key: keyof typeof form, value: string | boolean | RentCycle) => setForm((old) => ({ ...old, [key]: value }));
  ...
  const handleSave = async () => {
    ...
    if (!canManageLease) { Taro.showToast({ title: "当前角色没有管理租约权限", icon: "none" }); return; }
    if (!form.tenantName.trim() || !form.tenantPhone.trim() || !form.rentAmount.trim()) {
  ...
  return (
    <View className="page-container">
      ...
      <Card title="签约入住">
        <Input label="租客姓名" placeholder="请输入姓名" value={form.tenantName} onChange={(value) => updateForm("tenantName", value)} />
        <Input label="租客电话" placeholder="请输入手机号" value={form.tenantPhone} onChange={(value) => updateForm("tenantPhone", value)} />
        <View className="form-grid">
          <DateField label="开始日期" placeholder="选择日期" value={form.startDate} onChange={(value) => updateForm("startDate", value)} />
          <DateField label="结束日期" placeholder="选择日期" value={form.endDate} onChange={(value) => updateForm("endDate", value)} />
        </View>
        <View className="form-grid">
          <Input label="租金" placeholder="每期金额" type="number" value={form.rentAmount} onChange={(value) => updateForm("rentAmount", value)} />
          <Input label="押金" placeholder="请输入押金" type="number" value={form.depositAmount} onChange={(value) => updateForm("depositAmount", value)} />
        </View>
        <View className="form-grid">
          <Input label="宽限天数" placeholder="交租日后几日内" type="number" value={form.graceDays} onChange={(value) => updateForm("graceDays", value)} />
        </View>
        <Text className="field-label">交租周期</Text>
        <View className="segment">
          {(["MONTHLY", "QUARTERLY", "YEARLY"] as RentCycle[]).map((item) => (
            <View key={item} className={`segment-item ${form.cycle === item ? 'segment-item--active' : ''}`} onClick={() => updateForm("cycle", item)}>
        ...
        <View className="segment">
          {[true, false].map((item) => (
            <View key={String(item)} className={`segment-item ${form.autoRenew === item ? 'segment-item--active' : ''}`} onClick={() => updateForm("autoRenew", item)}>
        ...
        {new Date(form.startDate).setHours(0, 0, 0, 0) < new Date().setHours(0, 0, 0, 0) ? (
          <>
            ...
            <View className="segment">
              {[false, true].map((item) => (
                <View key={String(item)} className={`segment-item ${form.generateHistoricalBills === item ? 'segment-item--active' : ''}`} onClick={() => updateForm("generateHistoricalBills", item)}>
        ...
        <Button loading={saving} disabled={saving} onClick={handleSave}>确认签约</Button>
