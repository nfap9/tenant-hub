...
export default function RoomFormPage() {
  ...
  const [form, setForm] = useState({ roomNo: "", layout: "", area: "", facilities: "", status: "VACANT" as RoomStatus });
  ...
  const updateForm = (key: keyof typeof form, value: string | RoomStatus) => setForm((old) => ({ ...old, [key]: value }));
  ...
  const handleSave = async () => {
    ...
    if (!form.roomNo.trim() || !form.layout.trim()) { Taro.showToast({ title: "请填写房间号和户型", icon: "none" }); return; }
  ...
  return (
    <View className="page-container">
      ...
      <Card title="编辑房间">
        <Input label="房间号" placeholder="例如 301" value={form.roomNo} onChange={(value) => updateForm("roomNo", value)} />
        <View className="form-grid">
          <Input label="户型" placeholder="例如 开间" value={form.layout} onChange={(value) => updateForm("layout", value)} />
          <Input label="面积" placeholder="平方米" type="number" value={form.area} onChange={(value) => updateForm("area", value)} />
        </View>
        <FacilitySelector value={form.facilities} onChange={(value) => updateForm("facilities", value)} />
        <Text className="field-label">状态</Text>
        <View className="segment">
          {roomStatuses.map((item) => (
            <View key={item} className={`segment-item ${form.status === item ? 'segment-item--active' : ''}`} onClick={() => updateForm("status", item)}>
              <Text className={`segment-text ${form.status === item ? 'segment-text--active' : ''}`}>{statusLabels[item]}</Text>
            </View>
          ))}
        </View>
        <Button loading={saving} disabled={saving} onClick={handleSave}>保存房间信息</Button>
