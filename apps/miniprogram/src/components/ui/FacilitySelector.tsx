...
const defaultFacilityOptions = ["床", "衣柜", "空调", "热水器", "书桌", "椅子", "洗衣机", "冰箱", "宽带", "独卫", "阳台", "抽油烟机"];

const parseFacilities = (value: string) =>
  value.split(/[,，、]/).map((item) => item.trim()).filter(Boolean);

const stringifyFacilities = (items: string[]) => Array.from(new Set(items)).join(",");
  ...
  return (
    <View className="facility-selector">
      <View className="facility-selector__header">
        ...
        <Text className="facility-selector__count">已选 {selectedFacilities.length} 项</Text>
      </View>
      <View className="facility-selector__chips">
        {options.map((facility) => {
          ...
          return (
            ...
            >
              <Text className={`facility-chip__text ${selected ? 'facility-chip__text--selected' : ''}`}>{facility}</Text>
            </View>
          );
        })}
      </View>
      <View className="facility-selector__custom">
        ...
        />
        <View className={`facility-selector__add ${customValue.trim() ? '' : 'facility-selector__add--disabled'}`} onClick={addCustomFacility}>
      ...
      {selectedFacilities.length > 0 ? (
        <View className="facility-selector__selected">
          {selectedFacilities.map((facility) => (
            <View key={facility} className="facility-selected" onClick={() => toggleFacility(facility)}>
