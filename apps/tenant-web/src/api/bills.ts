...
export async function deleteMonthlyBill(organizationId: string, id: string) {
  return apiClient<void>(`/bills/monthly/${id}`, { method: "DELETE", organizationId });
}
...
export async function retryBillBilling(organizationId: string, billId: string) {
  return apiClient<void>(`/bills/${billId}/retry-billing`, { method: "POST", organizationId });
}

export async function createPayment(organizationId: string, payload: {
  ...
  return apiClient<Payment>(path, { method: "POST", body: payload, organizationId });
}

export async function updateBillItem(organizationId: string, billId: string, itemId: string, payload: {
  amount?: number;
  note?: string;
}) {
...
export async function recordUtilityReading(organizationId: string, billId: string, payload: {
...
export async function importUtilityCsv(organizationId: string, csvText: string) {
...
export async function createMeterReading(organizationId: string, payload: {
  apartmentId: string;
  roomId: string;
  leaseId?: string;
