...
interface LeaseTerminateDrawerProps {
  open: boolean;
  roomId: string;
  onCancel: () => void;
  onSuccess: () => void;
...
export default function LeaseTerminateDrawer({
  open,
  roomId,
  onCancel,
  onSuccess,
}: LeaseTerminateDrawerProps) {
      ...
      <Spin spinning={loading}>
        {hasActiveLease ? (
          <Form form={form} layout="vertical" onFinish={handleSubmit}>
            ...
            <Row gutter={[24, 0]} className="mb-16">
              <Col span={12}>
                <Form.Item label="房间押金退还" name="roomDepositRefundAmount">
                  <InputNumber min={0} className="w-full" prefix="¥" />
                </Form.Item>
              </Col>
              <Col span={12}>
                <Form.Item label="钥匙押金退还" name="keyDepositRefundAmount">
                  <InputNumber min={0} className="w-full" prefix="¥" />
                </Form.Item>
              </Col>
            </Row>
