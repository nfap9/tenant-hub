        ...
        <Card style={{ maxWidth: 720 }}>
          {lease ? (
            <Form form={form} layout="vertical" onFinish={handleSubmit}>
              ...
              <Divider orientation="left">押金与房租</Divider>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8, padding: "0 8px" }}>
                <span>原押金</span>
                <span style={{ fontWeight: 500 }}>¥{money(lease.depositAmount)}</span>
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16, padding: "0 8px" }}>
                <span>预计退押金</span>
                <span style={{ fontWeight: 500, color: "#52c41a" }}>¥{money(preview.depositRefund)}</span>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                ...
                <Form.Item label="房租退补" name="rentAdjustmentAmount">
                  <InputNumber style={{ width: "100%" }} prefix="¥" placeholder="正数补收，负数退款" />
                </Form.Item>
              </div>
              ...
              <Divider orientation="left">水电读数</Divider>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16, padding: "0 8px" }}>
                <span>预估水电费</span>
                <span style={{ fontWeight: 500, color: "#faad14" }}>¥{money(preview.utility)}</span>
              </div>
              ...
              <Divider orientation="left">结算预览</Divider>
              <div style={{ background: "#f5f5f5", padding: 16, borderRadius: 8, marginBottom: 16 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                  <span>应收</span>
                  <span style={{ fontWeight: 500 }}>¥{money(preview.receivable)}</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                  <span>应退</span>
                  <span style={{ fontWeight: 500 }}>¥{money(preview.refundable)}</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", paddingTop: 8, borderTop: "1px solid #ddd" }}>
                  <span>结算结果</span>
                  <span style={{ fontWeight: 600, color: preview.net > 0 ? "#52c41a" : preview.net < 0 ? "#ff4d4f" : "#666" }}>
              ...
              {lease.isAutoRenewalPeriod && (
                <Alert message="当前租约已进入自动续约期，到期后退房不默认视为违约。" type="info" showIcon style={{ marginBottom: 16 }} />
              )}

              <Form.Item>
                <Button type="primary" danger htmlType="submit" icon={<CheckOutlined />} loading={submitting} disabled={submitting}>
                  确认终止合约
                </Button>
