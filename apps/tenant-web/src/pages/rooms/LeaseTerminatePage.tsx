...
import { Card, Form, DatePicker, InputNumber, Input, Button, message, Spin, Radio, Divider, Alert } from "antd";
...
export default function LeaseTerminatePage() {
  ...
  useEffect(() => {
    if (lease && currentOrgId) {
      ...
      getSettlementPreview(currentOrgId!, lease.id, today())
        ...
        .catch((e) => message.error(e instanceof Error ? e.message : "退租读数加载失败"));
    }
  ...
  const preview = useMemo(() => {
    if (!lease) return { utility: 0, depositRefund: 0, receivable: 0, refundable: 0, net: 0 };
    return computeSettlementPreview(lease, {
  ...
  const handleSubmit = async (values: Record<string, unknown>) => {
    ...
    try {
      const settlement = await terminateLease(currentOrgId, lease.id, {
        ...
        reason: String(values.reason || "").trim() || terminationLabels[String(values.type) as keyof typeof terminationLabels],
        ...
        depositDeductionReason: String(values.depositDeductionReason || "").trim() || undefined,
  ...
  const netColor = preview.net > 0 ? "var(--th-success)" : preview.net < 0 ? "var(--th-danger)" : "var(--th-foreground-muted)";
          ...
          <Card>
            {lease ? (
              <Form form={form} layout="vertical" onFinish={handleSubmit}>
                <Form.Item label="退租类型" name="type" rules={[{ required: true }]}>
                  <Radio.Group
                    options={[
                      ...
                      { label: terminationLabels.NEGOTIATED, value: "NEGOTIATED" },
                      { label: terminationLabels.BREACH, value: "BREACH" },
                    ]}
                  />
                </Form.Item>
                <Form.Item label="退租日期" name="terminatedAt" rules={[{ required: true }]}>
                  <DatePicker style={{ width: "100%" }} size="large" prefix={<CalendarOutlined />} />
                </Form.Item>
                ...
                >
                  <span>原押金</span>
                  <span style={{ fontWeight: 500, color: "var(--th-foreground)" }}>¥{money(lease.depositAmount)}</span>
                </div>
                <div
                  style={{
                    display: "flex",
                ...
                >
                  <span>预计退押金</span>
                  <span style={{ fontWeight: 500, color: "var(--th-success)" }}>¥{money(preview.depositRefund)}</span>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                  ...
                  <Form.Item label="房租退补" name="rentAdjustmentAmount">
                    <InputNumber style={{ width: "100%" }} prefix="¥" placeholder="正数补收，负数退款" size="large" />
                  </Form.Item>
                </div>
                <Form.Item label="押金扣款原因" name="depositDeductionReason">
                  <Input placeholder="可选" size="large" prefix={<InfoCircleOutlined />} />
                </Form.Item>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                  <Form.Item label={`退租水表读数（上次 ${money(previousReadings.previousWater)}）`} name="currentWater">
                    <InputNumber min={0} style={{ width: "100%" }} size="large" />
                  </Form.Item>
                  <Form.Item label={`退租电表读数（上次 ${money(previousReadings.previousPower)}）`} name="currentPower">
                    <InputNumber min={0} style={{ width: "100%" }} size="large" />
                  </Form.Item>
                </div>
                <div
                ...
                >
                  <span>预估水电费</span>
                  <span style={{ fontWeight: 500, color: "var(--th-warning)" }}>¥{money(preview.utility)}</span>
                </div>
                ...
                >
                  ...
                  >
                    <span>应收</span>
                    <span style={{ fontWeight: 500, color: "var(--th-foreground)" }}>¥{money(preview.receivable)}</span>
                  </div>
                  <div
                    style={{
                      display: "flex",
                  ...
                  >
                    <span>应退</span>
                    <span style={{ fontWeight: 500, color: "var(--th-foreground)" }}>¥{money(preview.refundable)}</span>
                  </div>
                  <div
                    style={{
                      display: "flex",
                ...
                <Form.Item style={{ marginTop: 24 }}>
                  <Button type="primary" danger htmlType="submit" icon={<CheckOutlined />} loading={submitting} disabled={submitting} size="large">
                    确认终止合约
                  </Button>
