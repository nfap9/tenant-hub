...
import { SaveOutlined, LinkOutlined, InfoCircleOutlined } from "@ant-design/icons";
...
export default function OpsSmsConfigPage() {
    ...
    <Form.List name={name}>
      {(fields, { add, remove }) => (
        <div className="page-content">
          ...
          <Button type="dashed" onClick={() => add()} block size="large" style={{ borderRadius: "var(--th-radius)" }}>
            新增字段
          </Button>
        </div>
      )}
  ...
  return (
    <div className="page-content">
      ...
      >
        <Typography.Paragraph style={{ marginBottom: 8, color: "var(--th-foreground-muted)" }}>
          在 URL、Headers、Params 中均可使用以下变量，发送时会被自动替换为实际值：
        </Typography.Paragraph>
        <ul style={{ margin: 0, paddingLeft: 18, color: "var(--th-foreground)" }}>
          <li>
        ...
        <Typography.Paragraph
          type="secondary"
          style={{ marginTop: 12, marginBottom: 0, color: "var(--th-foreground-muted)" }}
        >
