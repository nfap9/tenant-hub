...
describe("LoginScreen", () => {
  ...
  it("should render login form by default", () => {
    const { getByText, getByPlaceholderText } = render(
      <LoginScreen phone="" setPhone={() => {}} mode="password" setMode={() => {}} onSignedIn={() => {}} />
    );
  ...
  it("should switch login mode between password and code", () => {
    ...
    const { getByText } = render(
      <LoginScreen phone="" setPhone={() => {}} mode="password" setMode={setMode} onSignedIn={() => {}} />
    );
  ...
  it("should call onSignedIn after successful login", async () => {
    ...
    const { getByPlaceholderText, getByText } = render(
      <LoginScreen phone="13800138000" setPhone={() => {}} mode="password" setMode={() => {}} onSignedIn={onSignedIn} />
    );
  ...
  it("should show error on login failure", async () => {
    ...
    const { getByPlaceholderText, getByText } = render(
      <LoginScreen phone="13800138000" setPhone={() => {}} mode="password" setMode={() => {}} onSignedIn={() => {}} />
    );
  ...
  it("should send OTP in code mode", async () => {
    ...
    const { getByText } = render(
      <LoginScreen phone="13800138000" setPhone={() => {}} mode="code" setMode={() => {}} onSignedIn={() => {}} />
    );

    fireEvent.press(getByText("获取验证码"));

    await waitFor(() => {
      expect(mockMobileApi).toHaveBeenCalledWith("/auth/otp", undefined, expect.objectContaining({ method: "POST" }));
    });
  });
