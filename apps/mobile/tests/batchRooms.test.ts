...
describe("batch rooms", () => {
  ...
  it("should reject excessive floor ranges", () => {
    expect(buildBatchRoomNos({ startFloor: "1", endFloor: "9999", roomCount: "99" })).toHaveLength(0);
  });

  it("should cap at max batch room count", () => {
    expect(buildBatchRoomNos({ startFloor: "1", endFloor: "20", roomCount: "10" })).toHaveLength(MAX_BATCH_ROOM_COUNT);
