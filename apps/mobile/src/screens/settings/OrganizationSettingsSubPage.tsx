  ...
  return (
    <>
      ...
      {memberships.length === 0 ? (
        <>
          <Card title="团队管理" subtitle="首次使用需要先创建组织，或输入管理员生成的邀请码加入已有团队。">
            {!onboardingAction ? (
              <>
        ...
        <>
          ...
          <Card title="调整成员角色">
            ...
            {managerRoles.map((role) => (
              <PressableScale key={role.id} scale={0.98} onPress={() =>
                run(async () => {
                ...
                <Card variant={roleEditingMember.roleId === role.id ? "default" : "outline"} padding="sm" gap={8}>
                  <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
          ...
          >
            ...
            {editingOrg ? (
              <View style={styles.detailPanel}>
                ...
                <Input multiline value={editDescription} onChangeText={setEditDescription} placeholder="组织描述" />
                <Button onPress={() =>
                  run(async () => {
            ...
            {currentMembership.organization.ownerId === currentUserId ? (
              <View style={styles.detailPanel}>
                ...
                <Input value={deleteConfirmName} onChangeText={setDeleteConfirmName} placeholder={currentMembership.organization.name} />
