...
export default function PlanPurchaseSubPage({
  ...
  const loadPlans = async () => {
    try {
      ...
      setPlans(nextPlans);
      if (currentOrgId) {
        const nextOverview = await mobileApi<SubscriptionOverview>(
  ...
  const endsAt = overview?.subscription?.endsAt
    ? new Date(overview.subscription.endsAt).toLocaleDateString()
    : '长期有效';
  ...
  return (
    <>
          ...
          <>
            ...
            <View style={styles.quotaRow}>
              <Text style={styles.quotaText}>
                公寓 {overview.usage.apartments}/
                {overview.subscription.plan.apartmentLimit + overview.extraQuota.apartmentQuota}
              </Text>
              <Text style={styles.quotaText}>
                成员 {overview.usage.members}/
                {overview.subscription.plan.memberLimit + overview.extraQuota.memberQuota}
              </Text>
            </View>
          </>
        ) : (
      ...
      <Card title="购买套餐">
        ...
        {plans.length === 0 ? (
          <Text style={styles.muted}>暂无可购买套餐，请联系运营端启用套餐。</Text>
        ) : null}
        {plans.map(plan => {
          ...
          return (
            ...
            >
              ...
              {!canManageOrg ? (
                <Text style={styles.muted}>需要组织管理权限才能购买套餐</Text>
              ) : null}
            </Card>
          );
        })}
          ...
          confirmingPlan ? (
            <View style={styles.roomActions}>
              <Button
                variant="ghost"
                size="small"
                onPress={() => setConfirmingPlanId(undefined)}
                icon="close-outline"
              >
                取消
              </Button>
              <Button
                size="small"
                loading={buyingPlanId === confirmingPlan.id}
                disabled={buyingPlanId === confirmingPlan.id}
                onPress={() => buy(confirmingPlan.id)}
                icon="cart-outline"
              >
      ...
      >
        {confirmingPlan ? (
          <>
            ...
            {overview?.subscription ? (
              <Text style={styles.muted}>当前套餐将更新为所选套餐。</Text>
            ) : null}
          </>
        ) : null}
      </TaskSheet>
