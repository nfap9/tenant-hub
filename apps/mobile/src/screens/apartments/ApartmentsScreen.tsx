...
const facilitiesText = (facilities?: string[]) =>
  facilities?.length ? facilities.join('、') : '未维护设施';
const toFacilityArray = (value: string) =>
  value
    .split(/[,，]/)
    .map(item => item.trim())
    .filter(Boolean);
...
export default function ApartmentsScreen({
  ...
  const selectedApartment = useMemo(
    () => apartments.find(item => item.id === selectedId),
    [apartments, selectedId],
  );
  const apartmentRooms = useMemo(
    () =>
      [...(selectedApartment?.rooms ?? [])].sort((left, right) =>
        left.roomNo.localeCompare(right.roomNo, 'zh-Hans-CN'),
      ),
    [selectedApartment],
  );
  const editingRoom = useMemo(
    () => apartmentRooms.find(room => room.id === editingRoomId),
    [apartmentRooms, editingRoomId],
  );
  const deletingRoom = useMemo(
    () => apartmentRooms.find(room => room.id === deleteRoomId),
    [apartmentRooms, deleteRoomId],
  );
  const expenseApartment = useMemo(
    () => apartments.find(item => item.id === expenseApartmentId) ?? selectedApartment,
    [apartments, expenseApartmentId, selectedApartment],
  );
  ...
  const updateForm = (key: keyof ApartmentForm, value: string) =>
    setForm(old => ({ ...old, [key]: value }));
  const updateRoomForm = (key: keyof RoomForm, value: string | RoomStatus) =>
  ...
  const saveApartment = async () => {
    ...
    try {
      ...
      const saved = await mobileApi<Apartment>(
        path,
        token,
        apiOptions(organizationId, method, payload),
      );
  ...
  const createRoom = async () => {
    ...
    try {
      const result = await mobileApi<{ count: number }>(
        `/apartments/${selectedApartment.id}/rooms/batch`,
        token,
        apiOptions(organizationId, 'POST', {
          rooms: [
            {
  ...
  const updateRoom = async () => {
    ...
    try {
      await mobileApi(
        `/apartments/rooms/${editingRoomId}`,
        token,
  ...
  const addBatchRooms = async () => {
    if (!organizationId || !selectedApartment) return;
    if (!canManageRoom) return setNotice('当前角色没有管理房间权限');
    const selectedRoomNos = generatedBatchRoomNos.filter(roomNo =>
  ...
  const renderLayoutSelector = (value: string, onChange: (layout: string) => void) => {
    const choices =
      value && !roomLayoutOptions.includes(value)
        ? [value, ...roomLayoutOptions]
        : roomLayoutOptions;
    return (
  ...
  const renderRoomFields = (showStatus = false) => (
    <>
      <View style={styles.formGrid}>
        <View style={styles.formField}>
          ...
          <Input
            ...
            onChangeText={value => updateRoomForm('roomNo', value)}
          />
        </View>
        <View style={styles.formField}>
          ...
          <Input
            ...
            onChangeText={value => updateRoomForm('area', value)}
          />
        </View>
      </View>
      <View style={styles.formField}>
      ...
      <Input
        ...
        onChangeText={value => updateRoomForm('facilities', value)}
      />
      {showStatus ? (
        <View style={styles.roleActions}>
  ...
  return (
    <>
            ...
            return (
              ...
              >
                ...
                <View style={styles.detailRow}>
                  ...
                  <Text style={styles.muted}>
                    水 {money(item.waterUnitPrice)} / 电 {money(item.powerUnitPrice)}
                  </Text>
                </View>
                <View style={styles.detailRow}>
        ...
        <>
          <View style={styles.subPageHeader}>
            <Button
              variant="ghost"
              size="small"
              onPress={mode === 'create' ? backToList : () => setMode('detail')}
          ...
          <Card title={mode === 'create' ? '新建公寓' : '编辑公寓信息'}>
            ...
            <View style={styles.formGrid}>
              <View style={styles.formField}>
                ...
                <Input
                  ...
                  value={form.floors}
                  keyboardType="numeric"
                  onChangeText={value => updateForm('floors', value)}
                />
              </View>
              <View style={styles.formField}>
                ...
                <Input
                  ...
                  value={form.landArea}
                  keyboardType="numeric"
                  onChangeText={value => updateForm('landArea', value)}
                />
              </View>
              <View style={styles.formField}>
                ...
                <Input
                  ...
                  keyboardType="numeric"
                  onChangeText={value => updateForm('totalArea', value)}
                />
              </View>
            </View>
            ...
            <Input
              ...
              onChangeText={value => updateForm('landlordName', value)}
            />
            <Input
              placeholder="联系方式"
              value={form.landlordPhone}
              onChangeText={value => updateForm('landlordPhone', value)}
            />
            <View style={styles.formGrid}>
              <View style={styles.formField}>
              ...
              <View style={styles.formField}>
                ...
                <Input
                  ...
                  keyboardType="numeric"
                  onChangeText={value => updateForm('rentAmount', value)}
                />
              </View>
            </View>
            ...
            <View style={styles.formGrid}>
              <View style={styles.formField}>
                ...
                <Input
                  ...
                  keyboardType="numeric"
                  onChangeText={value => updateForm('waterUnitPrice', value)}
                />
              </View>
              <View style={styles.formField}>
                ...
                <Input
                  ...
                  keyboardType="numeric"
                  onChangeText={value => updateForm('powerUnitPrice', value)}
                />
              </View>
            </View>
      ...
      {mode === 'detail' && selectedApartment ? (
        <>
          <View style={styles.subPageHeader}>
            <Button variant="ghost" size="small" onPress={backToList} icon="arrow-back-outline">
              返回公寓列表
            </Button>
          </View>
              ...
              canManageApartment ? (
                <View style={{ flexDirection: 'row', gap: 8 }}>
                  ...
                  <Button
                    variant="danger"
                    size="small"
                    onPress={() => setActiveLayer('apartmentDelete')}
                    icon="trash-outline"
                  >
                    删除
                  </Button>
          ...
          >
            <View style={styles.segment}>
              ...
              >
                <Text
                  style={[styles.segmentText, detailTab === 'expenses' && styles.segmentTextActive]}
                  onPress={() => {
                    resetRoomWork();
                    setDetailTab('expenses');
                  }}
              ...
              <View style={[styles.segmentItem, detailTab === 'rooms' && styles.segmentItemActive]}>
                <Text
                  style={[styles.segmentText, detailTab === 'rooms' && styles.segmentTextActive]}
                  onPress={() => {
                    setActiveLayer(undefined);
                    setExpenseApartmentId(undefined);
                    setDetailTab('rooms');
                ...
                <View style={styles.detailPanel}>
                  <View style={styles.detailRow}>
                    ...
                    <Text style={styles.cardTitle}>
                      {selectedApartment.floors} 层 · 占地 {selectedApartment.landArea ?? '未填'}㎡
                      · 总 {selectedApartment.totalArea ?? '未填'}㎡
                    </Text>
                  </View>
                  <View style={styles.detailRow}>
                    <Text style={styles.muted}>上游房东</Text>
                    <Text style={styles.cardTitle}>
                      {selectedApartment.landlordName || '未维护'} ·{' '}
                      {selectedApartment.landlordPhone || '未维护'}
                    </Text>
                  </View>
                  <View style={styles.detailRow}>
                  ...
                  <View style={styles.detailRow}>
                    ...
                    <Text style={styles.muted}>
                      水 ¥{money(selectedApartment.waterUnitPrice)} · 电 ¥
                      {money(selectedApartment.powerUnitPrice)}
                    </Text>
                  </View>
                  <View style={styles.detailRow}>
              ...
              >
                {(selectedApartment.expenses ?? []).slice(0, 4).map(item => (
                  <View style={styles.detailRow} key={item.id}>
                    <Text style={styles.muted}>
                      {item.name} · {toDateInput(item.spentAt)}
                    </Text>
                    <Text style={styles.cardStat}>¥{money(item.amount)}</Text>
                  </View>
                ))}
                {(selectedApartment.expenses ?? []).length === 0 ? (
                  <Text style={styles.muted}>暂无经营花费记录</Text>
                ) : null}
              </Card>
            </>
          ) : null}
          ...
          >
            {apartmentRooms.map(room => (
              <Card
                key={room.id}
                variant={
                  editingRoomId === room.id || deleteRoomId === room.id ? 'default' : 'outline'
                }
                padding="md"
                gap={10}
                style={
                  editingRoomId === room.id || deleteRoomId === room.id
                    ? {
                        borderWidth: 1.5,
            ...
            {apartmentRooms.length === 0 ? (
              <Text style={styles.muted}>暂无房间，可以新增单个房间或批量添加</Text>
            ) : null}
          </Card>
        </>
      ) : null}
      ...
      <TaskSheet
        ...
        footer={
          <Button onPress={() => addExpense()} icon="save-outline">
            保存花费
          </Button>
        }
      >
        <View style={styles.formGrid}>
          <Input
            style={{ flexGrow: 1, flexBasis: 104 }}
            placeholder="花费名称"
            value={expense.name}
            onChangeText={value => setExpense(old => ({ ...old, name: value }))}
          />
          <Input
            style={{ flexGrow: 1, flexBasis: 104 }}
            placeholder="金额"
            ...
            keyboardType="numeric"
            onChangeText={value => setExpense(old => ({ ...old, amount: value }))}
          />
          <DateField
            style={{ flexGrow: 1, flexBasis: 104 }}
            value={expense.spentAt}
            onChange={value => setExpense(old => ({ ...old, spentAt: value }))}
          />
        </View>
        <Input
          placeholder="备注"
          value={expense.note}
          onChangeText={value => setExpense(old => ({ ...old, note: value }))}
        />
      </TaskSheet>
      ...
      <TaskSheet
        ...
        footer={
          <Button onPress={createRoom} icon="save-outline">
            保存房间
          </Button>
        }
      >
      ...
      <TaskSheet
        ...
        onClose={resetRoomWork}
        footer={
          <Button onPress={addBatchRooms} icon="checkmark-outline">
            确认添加房间
          </Button>
        }
      >
        <View style={styles.formGrid}>
          <View style={styles.formField}>
            ...
            <Input
              ...
              keyboardType="numeric"
              onChangeText={setBatchStartFloor}
            />
          </View>
          <View style={styles.formField}>
          ...
          <View style={styles.formField}>
            ...
            <Input
              ...
              keyboardType="numeric"
              onChangeText={setBatchRoomCount}
            />
          </View>
        </View>
        <View style={styles.batchRoomPanel}>
          <View style={styles.detailRow}>
            ...
            <Text style={styles.muted}>
              已选 {selectedBatchRoomNos.length}/{generatedBatchRoomNos.length}
            </Text>
          </View>
              ...
              {generatedBatchRoomNos.map(roomNo => {
                ...
                return (
                  <Button
                    ...
                    size="small"
                    onPress={() =>
                      setSelectedBatchRoomNos(old => toggleBatchRoomSelection(old, roomNo))
                    }
                  >
                    {roomNo}
                  </Button>
        ...
        <Input
          ...
          onChangeText={setBatchFacilities}
        />
      </TaskSheet>
          ...
          deletingRoom ? (
            <View style={styles.roomActions}>
              <Button
                variant="danger"
                size="small"
                onPress={() => deleteRoom(deletingRoom)}
                icon="trash-outline"
              >
                确认删除
              </Button>
              <Button variant="ghost" size="small" onPress={resetRoomWork} icon="close-outline">
                取消
              </Button>
            </View>
          ) : null
      ...
      <TaskSheet
        ...
        subtitle={
          selectedApartment
            ? `${selectedApartment.name} · ${selectedApartment.location}`
            : undefined
        }
        ...
        footer={
          selectedApartment ? (
            <View style={styles.roomActions}>
              <Button variant="danger" size="small" onPress={deleteApartment} icon="trash-outline">
                确认删除
              </Button>
              <Button
                variant="ghost"
                size="small"
                onPress={() => setActiveLayer(undefined)}
                icon="close-outline"
              >
                取消
              </Button>
