...
const cycleLabels: Record<RentCycle, string> = {
  MONTHLY: '月付',
  QUARTERLY: '季付',
  YEARLY: '年付',
};
...
const defaultTerminationType = (lease: Lease): TerminationType =>
  today() > lease.endDate.slice(0, 10) ? 'EXPIRED' : 'NEGOTIATED';
const hasPermission = (membership: Membership | undefined, permission: string) =>
  Boolean(
    membership?.role.permissions.includes('*') || membership?.role.permissions.includes(permission),
  );
...
export default function RoomsScreen({
  ...
  const selectedRoom = useMemo(
    () => rooms.find(item => item.id === selectedId),
    [rooms, selectedId],
  );
  const editingRoom = useMemo(
    () => rooms.find(item => item.id === editingRoomId),
    [rooms, editingRoomId],
  );
  const leaseRoom = useMemo(
    () => rooms.find(item => item.id === leaseRoomId),
    [rooms, leaseRoomId],
  );
  const visibleRooms = useMemo(
    () => (filter === 'ALL' ? rooms : rooms.filter(item => item.status === filter)),
    [filter, rooms],
  );
  ...
  const settlementPreview = useMemo(() => {
    if (!terminatingLease)
      return { utility: 0, depositRefund: 0, receivable: 0, refundable: 0, net: 0 };
    ...
    const water =
      Math.max(numberValue(terminationForm.currentWater) - previousReadings.previousWater, 0) *
      Number(terminatingLease.waterUnitPrice ?? 0);
    const power =
      Math.max(numberValue(terminationForm.currentPower) - previousReadings.previousPower, 0) *
      Number(terminatingLease.powerUnitPrice ?? 0);
  ...
  const updateRoom = async () => {
    ...
    try {
      await mobileApi(
        `/apartments/rooms/${editingRoom.id}`,
        token,
  ...
  const deleteRoom = async () => {
    ...
    try {
      await mobileApi(
        `/apartments/rooms/${selectedRoom.id}`,
        token,
        apiOptions(organizationId, 'DELETE'),
      );
      setNotice('房间已删除');
      setSelectedId(undefined);
      setEditingRoomId(undefined);
  ...
  const createLease = async () => {
    ...
    if (
      ...
      !leaseForm.tenantPhone.trim() ||
      !leaseForm.rentAmount.trim()
    ) {
  ...
  const updateLease = async () => {
    ...
    try {
      await mobileApi(
        `/leases/${editingLease.id}`,
        token,
        apiOptions(organizationId, 'PUT', {
          rentAmount: editLeaseForm.rentAmount.trim()
            ? Number(editLeaseForm.rentAmount)
            : undefined,
          depositAmount: editLeaseForm.depositAmount.trim()
            ? Number(editLeaseForm.depositAmount)
            : undefined,
  ...
  const terminateLease = async () => {
    ...
    if (!canManageLease) return setNotice('当前角色没有管理租约权限');
    try {
      const settlement = await mobileApi<LeaseSettlement>(
        `/leases/${terminatingLease.id}/terminate`,
        token,
        apiOptions(organizationId, 'POST', {
      ...
      setNotice(
        net > 0
          ? `退租完成，租客应补交 ¥${money(net)}`
          : net < 0
            ? `退租完成，应退租客 ¥${money(Math.abs(net))}`
            : '退租完成，结算已结清',
      );
    ...
    <>
      {quickLeaseSelecting ? (
        <>
          <Card
            title="选择签约房间"
            subtitle="选择一间空闲房后填写租客和租约信息"
            headerAction={
              <Button
                variant="ghost"
                size="small"
                onPress={() => setQuickLeaseSelecting(false)}
          ...
          {leaseCandidateRooms.length === 0 ? (
            <EmptyState icon="🛏️" title="暂无空闲房间" subtitle="暂无空闲房间可签约" />
          ) : null}
                  ...
                  >
                    <View>
                      <Text style={styles.cardTitle}>
                        {room.apartment?.name} · {room.roomNo}
                      </Text>
        ...
        <>
          ...
          {canManageLease ? (
            <Button onPress={() => setQuickLeaseSelecting(true)} icon="document-text-outline">
              签约入住
            </Button>
          ) : null}
          ...
          {visibleRooms.length === 0 ? (
            <EmptyState icon="🛏️" title="暂无房间" subtitle="请先到公寓页批量添加" />
          ) : null}
                    ...
                    <View style={styles.roomHeader}>
                      ...
                      <View style={styles.roomHeaderBadges}>
                        ...
                        {roomActiveLease?.currentMonthBillGenerated ? (
                          <Badge tone="primary">
                            {getRoomBillGeneratedLabel(roomActiveLease.currentMonthBillLabel)}
                          </Badge>
                        ) : null}
                        {roomActiveLease?.currentMonthBillSettled ? (
                          <Badge tone="success">账单已结清</Badge>
                        ) : null}
                      </View>
                    </View>
                    <Text style={styles.muted}>
                      {room.facilities.length ? room.facilities.join('、') : '暂无设施'}
                    </Text>
                      ...
                      <>
                        <View style={styles.detailPanel}>
                          ...
                          <View style={styles.detailRow}>
                            ...
                            <Text style={styles.muted}>
                              水 ¥{money(room.apartment?.waterUnitPrice)} · 电 ¥
                              {money(room.apartment?.powerUnitPrice)}
                            </Text>
                          </View>
                        </View>
                          ...
                          <View style={styles.detailPanel}>
                            ...
                            <View style={styles.detailRow}>
                              ...
                              <Text style={styles.cardStat}>
                                ¥{money(roomActiveLease.rentAmount)}
                              </Text>
                            </View>
                            <View style={styles.detailRow}>
                              <Text style={styles.muted}>合同期</Text>
                              <Text style={styles.muted}>
                                {roomActiveLease.startDate.slice(0, 10)} 至{' '}
                                {roomActiveLease.endDate.slice(0, 10)}
                              </Text>
                            </View>
                            <View style={styles.detailRow}>
                            ...
                            <View style={styles.detailRow}>
                              ...
                              <Text style={styles.muted}>
                                {roomActiveLease.autoRenew
                                  ? roomActiveLease.isAutoRenewalPeriod
                                    ? '自动续约中'
                                    : '到期后自动续约'
                                  : '不自动续约'}
                              </Text>
                            ...
                            {canManageLease ? (
                              <View style={{ flexDirection: 'row', gap: 8 }}>
                                <Button
                                  variant="secondary"
                                  size="small"
                                  onPress={() => openEditLease(roomActiveLease)}
                                  icon="create-outline"
                                >
                                  编辑租约
                                </Button>
                                <Button
                                  variant="danger"
                                  size="small"
                                  onPress={() => openTermination(roomActiveLease)}
                                  icon="exit-outline"
                                >
                                  退租
                                </Button>
      ...
      <TaskSheet
        ...
        title="编辑房间"
        subtitle={
          editingRoom ? `${editingRoom.apartment?.name} · ${editingRoom.roomNo}` : undefined
        }
        onClose={() => setEditingRoomId(undefined)}
        footer={
          <Button onPress={updateRoom} icon="save-outline">
            保存房间信息
          </Button>
        }
      >
        <Input
          ...
          onChangeText={value => setRoomForm(old => ({ ...old, roomNo: value }))}
        />
        <View style={styles.formGrid}>
          <View style={styles.formField}>
            ...
            <Input
              ...
              onChangeText={value => setRoomForm(old => ({ ...old, layout: value }))}
            />
          </View>
          <View style={styles.formField}>
            ...
            <Input
              ...
              onChangeText={value => setRoomForm(old => ({ ...old, area: value }))}
            />
          </View>
        </View>
        <Input
          placeholder="设施，用逗号分隔"
          value={roomForm.facilities}
          onChangeText={value => setRoomForm(old => ({ ...old, facilities: value }))}
        />
        <View style={styles.segment}>
          {(Object.keys(statusLabels) as RoomStatus[]).map(item => (
            ...
            >
              <Text
                style={[styles.segmentText, roomForm.status === item && styles.segmentTextActive]}
      ...
      <TaskSheet
        ...
        footer={
          <Button onPress={createLease} icon="document-text-outline">
            确认签约
          </Button>
        }
      >
        <Input
          ...
          onChangeText={value => setLeaseForm(old => ({ ...old, tenantName: value }))}
        />
        <Input
          placeholder="租客电话"
          value={leaseForm.tenantPhone}
          onChangeText={value => setLeaseForm(old => ({ ...old, tenantPhone: value }))}
        />
        <View style={styles.formGrid}>
          <View style={styles.formField}>
            ...
            <DateField
              ...
              onChange={value => setLeaseForm(old => ({ ...old, startDate: value }))}
            />
          </View>
          <View style={styles.formField}>
            ...
            <DateField
              ...
              onChange={value => setLeaseForm(old => ({ ...old, endDate: value }))}
            />
          </View>
        </View>
        <View style={styles.formGrid}>
          <View style={styles.formField}>
            ...
            <Input
              ...
              keyboardType="numeric"
              onChangeText={value => setLeaseForm(old => ({ ...old, rentAmount: value }))}
            />
          </View>
          <View style={styles.formField}>
            ...
            <Input
              ...
              keyboardType="numeric"
              onChangeText={value => setLeaseForm(old => ({ ...old, depositAmount: value }))}
            />
          </View>
        </View>
        <View style={styles.formGrid}>
          <View style={styles.formField}>
            ...
            <Input
              placeholder="交租日后几日内"
              value={leaseForm.graceDays}
              keyboardType="numeric"
              onChangeText={value => setLeaseForm(old => ({ ...old, graceDays: value }))}
            />
          </View>
          <View style={styles.formField}>
            ...
            <View style={styles.segment}>
              {[true, false].map(item => (
                <View
                  ...
                  style={[
                    styles.segmentItem,
                    leaseForm.autoRenew === item && styles.segmentItemActive,
                  ]}
                >
                  <Text
                    style={[
                      styles.segmentText,
                      leaseForm.autoRenew === item && styles.segmentTextActive,
                    ]}
        ...
        <View style={styles.segment}>
          {(['MONTHLY', 'QUARTERLY', 'YEARLY'] as RentCycle[]).map(item => (
            ...
            >
              <Text
                style={[styles.segmentText, leaseForm.cycle === item && styles.segmentTextActive]}
        ...
        <View style={styles.formGrid}>
          <View style={styles.formField}>
            ...
            <Input
              placeholder="元/吨"
              value={leaseForm.waterUnitPrice}
              keyboardType="numeric"
              onChangeText={value => setLeaseForm(old => ({ ...old, waterUnitPrice: value }))}
            />
          </View>
          <View style={styles.formField}>
            ...
            <Input
              placeholder="元/度"
              value={leaseForm.powerUnitPrice}
              keyboardType="numeric"
              onChangeText={value => setLeaseForm(old => ({ ...old, powerUnitPrice: value }))}
            />
          </View>
        </View>
              ...
              <View style={styles.segment}>
                {[false, true].map(item => (
                  <View
                    ...
                    style={[
                      styles.segmentItem,
                      leaseForm.generateHistoricalBills === item && styles.segmentItemActive,
                    ]}
                  >
                    <Text
                      style={[
                        styles.segmentText,
                        leaseForm.generateHistoricalBills === item && styles.segmentTextActive,
                      ]}
                      onPress={() =>
                        setLeaseForm(old => ({ ...old, generateHistoricalBills: item }))
                      }
                    >
                      {item ? '生成全部' : '仅当前期'}
                    </Text>
            ...
            <View key={type} style={{ gap: 8 }}>
              <PressableScale onPress={() => togglePresetFee(type)}>
                <Card variant={selected ? 'default' : 'outline'} padding="sm" gap={8}>
                  ...
                  >
                    ...
                    <Text style={selected ? styles.cardStat : styles.muted}>
                      {selected ? `¥${money(selected.amount)}` : '点击添加'}
                    </Text>
                  </View>
                </Card>
              </PressableScale>
              {selected ? (
                <Input
                  ...
                  keyboardType="numeric"
                  onChangeText={value => updatePresetFeeAmount(type, value)}
                />
              ) : null}
            </View>
          );
          ...
          <View key={item.id} style={{ flexDirection: 'row', gap: 8, alignItems: 'center' }}>
            <View style={{ flex: 1 }}>
              <Input
                ...
                value={item.name}
                onChangeText={value =>
                  setCustomFees(old => old.map((f, i) => (i === index ? { ...f, name: value } : f)))
                }
              />
            </View>
            <View style={{ flex: 1 }}>
              <Input
                ...
                onChangeText={value =>
                  setCustomFees(old =>
                    old.map((f, i) => (i === index ? { ...f, amount: value } : f)),
                  )
                }
              />
            </View>
        ...
        <Button
          ...
          onPress={() =>
            setCustomFees(old => [
              ...old,
              { id: `${Date.now()}-${old.length}`, name: '', amount: '' },
            ])
          }
      ...
      <TaskSheet
        ...
        subtitle={
          editingLease
            ? `${editingLease.tenantName} · ${editingLease.room?.apartment?.name} · ${editingLease.room?.roomNo}`
            : undefined
        }
        onClose={() => {
          setEditingLease(undefined);
        ...
        footer={
          <Button onPress={updateLease} icon="save-outline">
            保存修改
          </Button>
        }
      >
        <View style={styles.formGrid}>
          <View style={styles.formField}>
            ...
            <Input
              placeholder="每期金额"
              value={editLeaseForm.rentAmount}
              keyboardType="numeric"
              onChangeText={value => setEditLeaseForm(old => ({ ...old, rentAmount: value }))}
            />
          </View>
          <View style={styles.formField}>
            <Text style={styles.fieldLabel}>押金</Text>
            <Input
              placeholder="押金金额"
              value={editLeaseForm.depositAmount}
              keyboardType="numeric"
              onChangeText={value => setEditLeaseForm(old => ({ ...old, depositAmount: value }))}
            />
          </View>
        </View>
        <View style={styles.formGrid}>
          <View style={styles.formField}>
            ...
            <Input
              placeholder="元/吨"
              value={editLeaseForm.waterUnitPrice}
              keyboardType="numeric"
              onChangeText={value => setEditLeaseForm(old => ({ ...old, waterUnitPrice: value }))}
            />
          </View>
          <View style={styles.formField}>
            <Text style={styles.fieldLabel}>电费单价</Text>
            <Input
              placeholder="元/度"
              value={editLeaseForm.powerUnitPrice}
              keyboardType="numeric"
              onChangeText={value => setEditLeaseForm(old => ({ ...old, powerUnitPrice: value }))}
            />
          </View>
        </View>
        <Text style={styles.label}>费用项目</Text>
      ...
      <TaskSheet
        ...
        subtitle={
          terminatingLease
            ? `${terminatingLease.tenantName} · ${terminatingLease.startDate.slice(0, 10)} 至 ${terminatingLease.endDate.slice(0, 10)}`
            : undefined
        }
        onClose={() => setTerminatingLease(undefined)}
        footer={
          <Button variant="danger" onPress={terminateLease} icon="exit-outline">
            确认终止合约
          </Button>
        }
      >
        <View style={styles.segment}>
          {(Object.keys(terminationLabels) as TerminationType[]).map(item => (
            <View
              ...
              style={[
                styles.segmentItem,
                terminationForm.type === item && styles.segmentItemActive,
              ]}
            >
              <Text
                style={[
                  styles.segmentText,
                  terminationForm.type === item && styles.segmentTextActive,
                ]}
        ...
        <DateField
          value={terminationForm.terminatedAt}
          onChange={value => setTerminationForm(old => ({ ...old, terminatedAt: value }))}
        />
        <View style={styles.detailPanel}>
          <View style={styles.detailRow}>
        ...
        <View style={styles.formGrid}>
          <View style={styles.formField}>
            ...
            <Input
              keyboardType="numeric"
              value={terminationForm.depositDeductionAmount}
              onChangeText={value =>
                setTerminationForm(old => ({ ...old, depositDeductionAmount: value }))
              }
            />
          </View>
          <View style={styles.formField}>
            ...
            <Input
              keyboardType="numeric"
              value={terminationForm.rentAdjustmentAmount}
              onChangeText={value =>
                setTerminationForm(old => ({ ...old, rentAdjustmentAmount: value }))
              }
            />
          </View>
        </View>
        <Input
          placeholder="押金扣款原因"
          value={terminationForm.depositDeductionReason}
          onChangeText={value =>
            setTerminationForm(old => ({ ...old, depositDeductionReason: value }))
          }
        />
        <View style={styles.formGrid}>
          <View style={styles.formField}>
            ...
            <Input
              keyboardType="numeric"
              value={terminationForm.currentWater}
              onChangeText={value => setTerminationForm(old => ({ ...old, currentWater: value }))}
            />
          ...
          <View style={styles.formField}>
            ...
            <Input
              keyboardType="numeric"
              value={terminationForm.currentPower}
              onChangeText={value => setTerminationForm(old => ({ ...old, currentPower: value }))}
            />
        ...
        <View style={styles.formGrid}>
          <View style={styles.formField}>
            ...
            <Input
              keyboardType="numeric"
              value={terminationForm.otherFeeAmount}
              onChangeText={value => setTerminationForm(old => ({ ...old, otherFeeAmount: value }))}
            />
          </View>
          <View style={styles.formField}>
        ...
        <Input
          placeholder="其他费用说明"
          value={terminationForm.otherFeeReason}
          onChangeText={value => setTerminationForm(old => ({ ...old, otherFeeReason: value }))}
        />
        <View style={styles.detailPanel}>
          <View style={styles.detailRow}>
          ...
          <View style={styles.detailRow}>
            ...
            <Text style={settlementPreview.net >= 0 ? styles.cardStat : styles.smallDangerText}>
              {settlementPreview.net > 0
                ? `租客补交 ¥${money(settlementPreview.net)}`
                : settlementPreview.net < 0
                  ? `退租客 ¥${money(Math.abs(settlementPreview.net))}`
                  : '结清'}
            </Text>
          </View>
        </View>
        ...
        <Input
          multiline
          placeholder="可选，默认使用解约类型"
          value={terminationForm.reason}
          onChangeText={value => setTerminationForm(old => ({ ...old, reason: value }))}
        />
        {terminatingLease?.isAutoRenewalPeriod ? (
          <Text style={styles.muted}>当前租约已进入自动续约期，到期后退房不默认视为违约。</Text>
        ) : null}
      </TaskSheet>
    </>
  );
