...
import type {
  ...
  RoomActionKey,
} from '../navigation/homeQuickActions';
            ...
            >
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                ...
                <View style={styles.userPillText}>
                  ...
                  <Text style={styles.userOrgName}>
                    {currentMembership?.organization.name ?? '尚未选择组织'}
                  </Text>
                </View>
              </View>
            </PressableScale>
          ...
          <View style={styles.userMenu}>
            ...
            {memberships.map(item => (
              <PressableScale
                ...
                style={[
                  styles.menuOrgItem,
                  currentOrgId === item.organization.id && styles.menuOrgItemActive,
                ]}
                onPress={() => {
              ...
              >
                <View>
                  ...
                  <Text style={styles.userMenuPhone}>
                    {item.role.name} · {item.organization.code}
                  </Text>
                </View>
                {currentOrgId === item.organization.id ? (
                  <Text style={styles.link}>当前</Text>
                ) : null}
              </PressableScale>
            ))}
            <View style={styles.menuDivider} />
            <Button variant="danger" size="small" onPress={signOut}>
              退出登录
            </Button>
          </View>
        ) : null}
        ...
        >
          <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
            ...
            {memberships.length === 0 ? (
              <Card
                title="开始使用 Tenant Hub"
                subtitle="你还没有加入组织。先创建自己的组织，或输入管理员生成的邀请码加入已有团队。"
              >
