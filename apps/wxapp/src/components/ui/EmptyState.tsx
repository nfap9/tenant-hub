...
export function EmptyState({ icon = 'bill', emoji, title, subtitle, action }: EmptyStateProps) {
  return (
    <View className="empty-state">
      <View className="empty-state__icon">
        {emoji ? <Text className="empty-state__emoji">{emoji}</Text> : <Icon name={icon} size="xl" />}
      </View>
