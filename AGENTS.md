...
- **`apps/wxapp`** — 最终用户微信小程序（Taro 4 + React 18），承载组织、公寓、房间、租约、账单、水电录入、收款等全部业务操作
...
### 2.3 微信小程序 (`apps/wxapp`)
- **框架**：Taro 4.0.9 + React 18
- **构建**：`pnpm dev:weapp` 启动微信开发者工具热更新；`pnpm build:weapp` 生产构建
- **样式**：Sass
...
# 小程序开发（启动 Taro 微信构建）
pnpm dev:wxapp         # 等价于 pnpm --filter @tenant-hub/wxapp dev:weapp
...
pnpm --filter @tenant-hub/wxapp dev:weapp
```
...
| 小程序入口 | `apps/wxapp/src/app.tsx` |
| 小程序路由配置 | `apps/wxapp/src/app.config.ts` |
