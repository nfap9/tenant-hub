...
- **导航**：未使用 React Navigation，采用自定义 Tab 导航壳（`MainTabBar` + `AppRoot` 状态驱动页面切换）
...
│       │   ├── navigation/     # MainTabBar, tabs.ts, homeQuickActions.ts
│       │   ├── services/       # http.ts（fetch 封装）、session.ts
│       │   ├── theme/          # tokens（colors/spacing/typography）+ styles（StyleSheet）
│       │   ├── types/          # 与 Prisma 模型对应的 TypeScript 类型
│       │   └── constants/      # config.ts（API_BASE_URL 解析）
│       ├── __tests__/          # Jest 单元测试（components, screens, app）
│       ├── tests/              # 遗留/源码静态测试（Jest 同时匹配）
│       ├── metro.config.js     # Monorepo watchFolders + nodeModulesPaths + .env cacheVersion
│       ├── babel.config.js     # react-native-dotenv 插件配置
│       └── jest.config.js      # Jest 29 配置（覆盖率阈值 30%）
├── scripts/
...
- **单元测试**：Jest 29 + `@testing-library/react-native`。
  - 配置在 `jest.config.js`，覆盖率阈值统一为 **30%**。
  - 测试文件匹配 `**/__tests__/**/*.test.ts` 与 `.tsx`，以及 `**/tests/**/*.test.ts`。
  - `jest.setup.js` 为空占位（`@testing-library/react-native` v13+ matchers 自动注入）。
  - `__mocks__/react-native-dotenv.js` 提供测试环境 `API_BASE_URL`。
  - 运行：`pnpm mobile:test`
- **遗留源码静态测试**：`tests/` 目录下的 `.test.ts` 文件多为源码静态分析测试（读取文件内容做正则匹配），同样由 Jest 执行。
...
- **触发条件**：`push` / `pull_request` 到 `main`/`master`，且变更路径为 `apps/mobile/**`、工作流文件、`tsconfig.base.json`、`eslint.config.mjs`、`package.json`、`pnpm-lock.yaml`、`pnpm-workspace.yaml`。
- **Job 1 — 检查**：Typecheck → Lint → Format Check → Test（Jest）
